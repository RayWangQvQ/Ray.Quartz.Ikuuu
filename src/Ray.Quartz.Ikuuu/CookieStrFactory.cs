...
{
    public class CookieStrFactory : ISingletonDependency
    {
        private readonly Dictionary<int, Dictionary<string, string>> _cookieDictionary;

        public CookieStrFactory() :this(new List<string>())
        {
        }
        ...
        public void Add(string ckStr)
        {
            var newKey= _cookieDictionary.Keys.LastOrDefault()+1;
            _cookieDictionary.Add(newKey, CkStrToDictionary(ckStr));
        }
        ...
        private Dictionary<string, string> CkStrToDictionary(string ckStr)
        {
            ...
            var ckItemList = ckStr.Split(";", StringSplitOptions.TrimEntries| StringSplitOptions.RemoveEmptyEntries).Distinct();
            foreach (var item in ckItemList)
            {
                ...
                dic.AddIfNotExist<string, string>(key, value);
            }
            return dic;
        }
